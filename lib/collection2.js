...
Mongo.Collection.prototype.attachSchema = function c2AttachSchema(ss, options) {
  var self = this;
  options = options || {};
  var selector = options.selector && options.selector;
  ...
  if (typeof selector === "object") {
    ...
    //ss = { schema: new SimpleSchema(ss), selector: selector };

    // we need an array to hold multiple schemas
    if (!Array.isArray(self._c2._simpleSchemas)) {
      self._c2._simpleSchemas = [];
    }
    self._c2._simpleSchemas.push({
      schema: new SimpleSchema(ss), selector: selector
    });
  } else {
  ...
  if (self._collection instanceof LocalCollection) {
    ...
    if (!selector) {
      ...
      self._collection._c2._simpleSchemas.push({
        schema: new SimpleSchema(ss), selector: selector
      });
    }
  }
...
_.each([Mongo.Collection, LocalCollection], function (obj) {
  obj.prototype.simpleSchema = function (doc, options) {
    var self = this;
    if (self._c2) {
      if (typeof self._c2._simpleSchema === "object") {
        return self._c2._simpleSchema;
      } else if (typeof self._c2._simpleSchemas === "object" &&
        self._c2._simpleSchemas.length > 0) {
        if (!doc || !options) {
          throw new Meteor.Error(400, "Arguments missed",
            "Document object or selector has not been transferred for" +
            " collection.simpleSchema() call");
        }
        var schemas = self._c2._simpleSchemas;
        for (var i = 0; i < schemas.length; i++) {
          var selector = Object.keys(schemas[i].selector)[0];
          var target = null;
          if (typeof doc[selector] !== 'undefined' || (doc.$set &&
            typeof doc.$set[selector] !== 'undefined')) {
            // $set should have more priority here
            target = (doc.$set && doc.$set[selector]) || doc[selector];
...
function doValidate(type, args, getAutoValues, userId, isFromTrustedCode) {
  ...
  // For `_simpleSchemas`: in case when `update` and selector given we have to
  // check availability of `selector` within doc. If we have new `selector` -
  // this probably means - user want to change schema for this doc, so, we need
  // to get all doc to clean and revalidate it.
  //if (type === "update" && options.selector) {
  //  var opSelector = Object.keys(options.selector)[0];
  //  if (doc.$set[opSelector] !== options.selector[opSelector]) {
  //    var newDoc = self.findOne(selector);
  //    //var newDocInSet = ;
  //    //newDocInSet["$set"] = newDoc;
  //    console.log(Object.assign);
  //    doc["$set"] = Object.assign({}, newDoc, doc.$set);
  //  }
  //}
